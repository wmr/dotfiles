...
function __is_local {
  if [[ ${__LAST_PWD} != ${PWD} ]]; then
    if $(is_remote 2>/dev/null); then
      export __PWD_IS_REMOTE=1
    else
      unset __PWD_IS_REMOTE
    fi
    export __LASTPWD=${PWD}
  fi
  return ${__PWD_IS_REMOTE}
}
...
function __promptline_right_prompt {
  ...
  # section "c" header
  slice_prefix="${c_sep_fg}${rsep}${c_fg}${c_bg}${space}" slice_suffix="$space${c_sep_fg}" slice_joiner="${c_fg}${c_bg}${alt_rsep}${space}" slice_empty_prefix=""
  # section "c" slices
